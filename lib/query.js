  ...
  execCommand(application) {
    ...
    if (typeof func !== "undefined") {
      if (application.$method[this.command] === "command") {
        if (typeof func === "function") {
          try {
            ...
            if (result instanceof Promise) {
              ...
              return result
                ...
                .catch((error) => {
                  ...
                  if (error instanceof Error) {
                    ...
                    this.res.send({ code: "error", info: JSON.stringify(error, Object.getOwnPropertyNames(error)) })
                  }
                });
              ...
              this.res.send({
                code: "error",
                info: `Command ${this.command} no return`,
              });
            }
          } catch (e) {
            // execution error
            ...
            this.res.send({
              code: "error",
              info: `Unable to execute command ${this.command} : ${e}`,
            });
          }
        } else {
          // simple value
        ...
        this.res.send({
          code: "error",
          info: `Command ${this.command} is private`,
        });
      }
    } else {
      // undefined command
      this.res.send({
        code: "error",
        info: `Command ${this.command} is not defined`,
      });
    }
  }
};